...
cotas = invest_info.groupby(["Data", "Bloomberg ID", "Fundo", "Tipo"]).sum()
cotas = cotas.reset_index().pivot(index="Data", columns='Bloomberg ID', values='Quantidade de cotas')
...
with tab1:
    ...
    consolidado = px.bar(curves_cons, x="Date", y="AUM", title="AUM Consolidado",color_discrete_sequence=["gold"], labels={"Date": ""})
    consolidado.add_scatter(x=[curves_cons["Date"].iloc[-1]], y=[curves_cons["AUM"].iloc[-1] * 1.1], mode="text",
